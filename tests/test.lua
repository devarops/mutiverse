...
describe("apply_mutations_from_plan", function()
    it("should apply a mutation from plan to source file", function()
        local report_path = os.tmpname()
        os.remove(report_path)
        local mutants = mutator.apply_mutations_from_plan("tests/data/mutation-plan.json", "true", report_path)
        os.remove(report_path)
        assert_contains(mutants[1], "    return 0")
    end)
    ...
    it("should write a partial report when processing stops early", function()
        local report_path = os.tmpname()
        os.remove(report_path)
        local ok, _ = pcall(mutator.apply_mutations_from_plan, "tests/data/mutation-plan-partial.json", "true", report_path)
        assert.is_falsy(ok)
        local content = file_io.read(report_path)
